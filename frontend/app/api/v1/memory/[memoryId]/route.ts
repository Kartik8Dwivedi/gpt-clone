...
export async function PUT(request: Request, { params }: { params: { memoryId: string } }) {
  const { userId } = auth();
  if (!userId) {
    logger.warn("Unauthorized request to update memory.");
    return NextResponse.json(new AppError("Unauthorized", 401), { status: 401 });
  }

  const { memoryId } = params;
  if (!Types.ObjectId.isValid(memoryId)) {
    return NextResponse.json(new AppError("Invalid Memory ID", 400), { status: 400 });
  }
  ...
  return handleRequest(() => memoryService.updateMemory(memoryId, value));
}

export async function DELETE(request: Request, { params }: { params: { memoryId: string } }) {
  const { userId } = auth();
  if (!userId) {
    logger.warn("Unauthorized request to delete memory.");
    return NextResponse.json(new AppError("Unauthorized", 401), { status: 401 });
  }
  ...
  return handleRequest(() => memoryService.deleteMemory(memoryId));
}
